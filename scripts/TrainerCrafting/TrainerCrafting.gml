/// @description Crafting trainer: menu, choices, smelt/craft gate copy.

function TrainerCrafting_Register(_inst) {
	with (_inst) {
			...
			array_push(choices, {
				...
				action: function() {
					if (instance_exists(self.trainer)) {
						with (self.trainer) { TeachCrafting(); }
					}
				}
			});
			array_push(choices, {
				...
				action: function() {
					if (instance_exists(self.trainer)) {
						with (self.trainer) { ExplainCrafting(); }
					}
				}
			});
