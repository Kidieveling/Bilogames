/// @description Mining trainer: menu, choices, pickaxe grant copy.

function TrainerMining_Register(_inst) {
	with (_inst) {
		...
		BuildMiningChoices = function() {
			...
			array_push(choices, {
				...
				action: function() {
					if (instance_exists(self.trainer)) {
						with (self.trainer) { ExplainMiningOrder(); }
					}
				}
			});
