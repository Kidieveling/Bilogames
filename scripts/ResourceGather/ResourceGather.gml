/// @description Data-driven gather loop: requirements, depletion, auto-gather; bound on obj_resource.

function ResourceGather_Register(_inst) {
	with (_inst) {
		depth = -y;
		...
		DepleteResource = function() {
			if (active_sprite == -1) active_sprite = sprite_index;
			if (active_resource_action == "") active_resource_action = resource_action;
			depleted = true;
			StopGathering();
		...
		RespawnResource = function() {
			...
			respawn_timer = 0;
			if (active_sprite != -1) sprite_index = active_sprite;
			if (active_resource_action != "") resource_action = active_resource_action;
		};
