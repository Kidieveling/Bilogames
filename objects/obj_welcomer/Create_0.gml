...
UpdateWelcomerAnimation = function() {
	...
	if (_moved) {
		...
		if (walk_anim_frame >= walk_frames) {
			walk_anim_frame -= walk_frames
		}
...
OpenDialogueMenu = function() {
	...
	DialogueWelcomer_BeginSession(id)
}

AcceptSecondChanceTrial = function() {
	if (GuidedIntro_IsTourActive(id)) {
		GuidedIntro_EndTour(id);
	}
	DialogueConversation_Start(id, DialogueWelcomer_AcceptTrialBeats(id))
}
